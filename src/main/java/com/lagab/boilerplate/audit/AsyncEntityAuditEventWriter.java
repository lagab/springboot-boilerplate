...
import com.lagab.boilerplate.jpa.domain.Identifiable;
import com.lagab.boilerplate.jpa.domain.audit.AbstractAuditingEntity;
import com.lagab.boilerplate.jpa.domain.audit.EntityAuditEvent;
import com.lagab.boilerplate.repository.EntityAuditEventRepository;
...
public class AsyncEntityAuditEventWriter {
    ...
    public void writeAuditEvent(Object target, EntityAuditAction action) {
        log.trace("-------------- Post {} audit  --------------", action.value());
        try {
    ...
    private EntityAuditEvent prepareAuditEntity(final Object entity, EntityAuditAction action) {
        ...
        try {
            if (entity instanceof Identifiable) {
                entityId = (Long) ((Identifiable) entity).getId();
            } else{
        ...
        if (EntityAuditAction.CREATE.equals(action)) {
            ...
            auditedEntity.setModifiedDate(abstractAuditEntity.getCreatedAt());
            auditedEntity.setCommitVersion(1);
        } else {
            auditedEntity.setModifiedBy(abstractAuditEntity.getModifiedBy());
            auditedEntity.setModifiedDate(abstractAuditEntity.getModifiedAt());
